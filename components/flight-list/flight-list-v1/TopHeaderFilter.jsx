import { useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { setSort } from '../../../features/flightSearch/flightSearchSlice';

const TopHeaderFilter = () => {
  const dispatch = useDispatch();
  const sort = useSelector((state) => state.flightSearch.value.sort);
  const [openSort, setOpenSort] = useState(false);
  return (
    <>
      <div className='row y-gap-10 items-center justify-between'>
        <div className='col-auto'>
          <div className='text-18'>
            <span className='fw-500'>3,269 properties</span> in Europe
          </div>
        </div>
        {/* End .col */}

        <div className='col-auto'>
          <div className='row x-gap-20 y-gap-20'>
            <div className='col-auto flight-search-sort-container'>
              <button
                className='button -blue-1 h-40 px-20 rounded-100 bg-blue-1-05 text-15 text-blue-1'
                onClick={() => setOpenSort((prev) => !prev)}
              >
                <i className='icon-up-down text-14 mr-10' />
                Sort
              </button>
              {openSort && (
                <div className='flight-search-sort bg-white px-15 py-20'>
                  <h5 className='text-18 fw-500 mb-10'>Sort</h5>
                  {sort &&
                    Object.entries(sort).map(([key, value], index) => (
                      <div className='form-checkbox d-flex items-center' key={index}>
                        <input
                          type='checkbox'
                          name='name'
                          checked={value}
                          onChange={() => dispatch(setSort({ key, value: !value }))}
                        />
                        <div className='form-checkbox__mark'>
                          <div className='form-checkbox__icon icon-check' />
                        </div>
                        <div className='text-15 ml-10'>
                          {key
                            .split('_')
                            .map(
                              (el, i) => `${el.charAt(0).toUpperCase()}${el.slice(1)} `
                            )}
                        </div>
                      </div>
                    ))}
                </div>
              )}
            </div>
            {/* End .col */}

            <div className='col-auto d-none xl:d-block'>
              <button
                data-bs-toggle='offcanvas'
                data-bs-target='#listingSidebar'
                className='button -blue-1 h-40 px-20 rounded-100 bg-blue-1-05 text-15 text-blue-1'
              >
                <i className='icon-up-down text-14 mr-10' />
                Filter
              </button>
            </div>
            {/* End .col */}
          </div>
          {/* End .row */}
        </div>
        {/* End .col */}
      </div>
      {/* End .row */}
    </>
  );
};

export default TopHeaderFilter;
