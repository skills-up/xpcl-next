...
const ViewCommissionRule = () => {
  ...
  useEffect(() => {
    // Getting particular CommissionRules
    getCommissionRule();
  }, [router.isReady]);

  const getCommissionRule = async () => {
    if (router.query.view) {
      const response = await getItem('commission-rules', router.query.view);
      if (response?.success) {
        ...
        setCommissionRule(data);
      } else {
        sendToast(
          'error',
          response.data?.message ||
            response.data?.error ||
            'Could Not Fetch The Requested Commission Rule.'
        );
  ...
  return (
    <>
      <Seo pageTitle='View Commission Rule' />
      {/* End Page Title */}

      <div className='row y-gap-20 justify-between items-end pb-60 lg:pb-40 md:pb-32'>
                <div className='col-12'>
                  <h1 className='text-30 lh-14 fw-600'>View Commission Rule</h1>
                  <div className='text-15 text-light-1'>
                    Get extended details of a commission rule.
                  </div>
                </div>
                {/* End .col-12 */}
              </div>
              ...
              <div className='py-30 px-30 rounded-4 bg-white shadow-3'>
                ...
                <ViewTable
                  data={commissionRule}
                  onEdit={() =>
                    router.push('/dashboard/commission-rules/edit/' + router.query.view)
                  }
                  onDelete={() => {
                    setIdToDelete(router.query.view);
                    setConfirmDelete(true);
                  }}
                  entitySlug={'commission-rules'}
                />
              </div>
              </>
  );
...
ViewCommissionRule.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;

export default ViewCommissionRule;
