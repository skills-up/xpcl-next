...
const UpdateCommissionRule = () => {
  ...
  const getData = async () => {
    if (router.query.edit) {
      const response = await getItem('commission-rules', router.query.edit);
      if (response?.success) {
        setIataBasic(response.data?.iata_basic);
        setIataYQ(response.data?.iata_yq);
        setPlbBasic(response.data?.plb_basic);
        setPlbYQ(response.data?.plb_yq);
        setName(response.data?.name);
        if (response.data?.start_date && response.data?.end_date) {
          setAllowDates(true);
          setDates([
            new DateObject({ date: response.data?.start_date, format: 'YYYY-MM-DD' }),
            new DateObject({ date: response.data?.end_date, format: 'YYYY-MM-DD' }),
          ]);
        }
      } else {
        sendToast(
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const response = await updateItem('commission-rules', router.query.edit, {
      name,
    ...
    if (response?.success) {
      sendToast('success', 'Updated Commission Rule Successfully.', 4000);
      router.push('/dashboard/commission-rules');
    } else {
      sendToast(
        'error',
  ...
  return (
    <>
      <Seo pageTitle='Update Commission Rule' />
      {/* End Page Title */}

      <div className='row y-gap-20 justify-between items-end pb-60 lg:pb-40 md:pb-32'>
                <div className='col-12'>
                  <h1 className='text-30 lh-14 fw-600'>Update Commission Rule</h1>
...
UpdateCommissionRule.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;

export default UpdateCommissionRule;
