...
const UpdateTravelMembership = () => {
  ...
  const getData = async () => {
    if (router.query.edit) {
      const response = await getItem('travel-memberships', router.query.edit);
      if (response?.success) {
  ...
  const onSubmit = async (e) => {
    ...
    if (membershipType?.value) {
      const response = await updateItem('travel-memberships', router.query.edit, {
      ...
      if (response?.success) {
        sendToast('success', 'Updated Travel Membership Successfully.', 4000);
        router.push('/dashboard/travellers/view/' + router.query.traveller_id);
      } else {
        sendToast(
          'error',
          response.data?.message ||
            response.data?.error ||
            'Failed to Update Travel Membership.',
          4000
        );
      }
    } else {
  ...
  return (
    <>
      <Seo pageTitle='Update Travel Membership' />
      {/* End Page Title */}

      <div className='row y-gap-20 justify-between items-end pb-60 lg:pb-40 md:pb-32'>
                <div className='col-12'>
                  <h1 className='text-30 lh-14 fw-600'>Update Travel Membership</h1>
                  <div className='text-15 text-light-1'>
                    Update an existing travel membership.
                  </div>
                </div>
                {/* End .col-12 */}
              </div>
...
UpdateTravelMembership.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;

export default UpdateTravelMembership;
