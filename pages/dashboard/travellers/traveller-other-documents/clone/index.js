...
const CloneTravellerOtherDocument = () => {
  ...
  const getData = async () => {
    if (router.query.clone) {
      const response = await getItem('traveller-other-documents', router.query.clone);
      if (response?.success) {
...
CloneTravellerOtherDocument.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;

export default CloneTravellerOtherDocument;
