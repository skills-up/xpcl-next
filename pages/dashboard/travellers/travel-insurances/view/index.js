...
const ViewTravelInsurances = () => {
  ...
  const getTravelInsurance = async () => {
    if (router.query.view) {
      const response = await getItem('travel-insurances', router.query.view);
      if (response?.success) {
        ...
        sendToast(
          ...
          response.data?.message ||
            response.data?.error ||
            'Could Not Fetch The Requested Travel Insurance.'
        );
  ...
  return (
    <>
      <Seo pageTitle='View Travel Insurance' />
      {/* End Page Title */}

      <div className='row y-gap-20 justify-between items-end pb-60 lg:pb-40 md:pb-32'>
                <div className='col-12'>
                  <h1 className='text-30 lh-14 fw-600'>View Travel Insurance</h1>
                  <div className='text-15 text-light-1'>
                    Get extended details of a travel insurance.
                  </div>
                </div>
                {/* End .col-12 */}
              </div>
...
ViewTravelInsurances.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;

export default ViewTravelInsurances;
